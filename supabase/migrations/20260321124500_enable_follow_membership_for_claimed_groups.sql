...
set
	...
	policy_markdown = case
		when coalesce(btrim(p.policy_markdown), '') = '' then
			'By joining, you agree to this membership policy.
...
- Contact group organizers through the group page.'
		else p.policy_markdown
	end,
	policy_updated_at = case
		when coalesce(btrim(p.policy_markdown), '') = '' then timezone('utc', now())
		else p.policy_updated_at
	end,
