...
<div class="mx-auto flex w-full max-w-7xl flex-col gap-6">
	<section class="card preset-tonal-surface space-y-4 p-5">
		<div class="flex flex-wrap items-center justify-between gap-2">
			<div>
				<p class="label opacity-60">Embeddable directory</p>
				<h1 class="text-2xl font-bold">Ride widget builder</h1>
				<p class="text-sm opacity-75">
					Configure filters and styles, then copy the iFrame code to embed this directory on your
					site.
				</p>
			</div>
			<div class="flex flex-wrap gap-2">
				<a class="btn preset-outlined-surface-950-50 gap-2" href={previewPath} target="_blank">
					Open preview
		...
		<div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
			...
			<label class="flex flex-col gap-1.5">
				...
				<input class="input" bind:value={radiusMiles} type="number" min="1" max="500" step="0.5" />
			</label>
		...
		<div class="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
			...
			<label
				class="toggle-row border-surface-500/20 flex items-center gap-3 rounded-xl border p-3"
			>
				<button
					type="button"
		...
		<label class="toggle-row border-surface-500/20 flex items-center gap-3 rounded-xl border p-3">
			<button
				type="button"
			...
			<span>Prefix ride titles with city name when available</span>
		</label>
	</section>

	<section class="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)]">
		<div class="card preset-tonal-surface overflow-hidden p-3">
			<div class="mb-2 flex items-center justify-between gap-2 px-2">
				<h2 class="text-lg font-semibold">Live preview</h2>
				<p class="text-xs opacity-65">Updates as you change settings</p>
			</div>
			<iframe
				title="Ride widget preview"
				src={previewPath}
				style={`height:${iframeHeight}px;`}
				class="border-surface-500/25 bg-surface-50-950 w-full rounded-xl border"
			></iframe>
		</div>

		<div class="card preset-tonal-surface flex flex-col gap-4 p-4">
			<div>
				<p class="label opacity-60">Share URL</p>
				<div
					class="border-surface-500/25 bg-surface-50-950/40 mt-1 overflow-x-auto rounded-lg border p-2 text-xs"
				>
					{shareUrl}
				</div>
				...
				{#if fullPreviewUrl.length > 280}
					<p class="mt-2 text-xs opacity-65">
						This URL is long. Use "Save short URL" to generate a single `wid` parameter.
					</p>
				{/if}
			</div>

			<div>
				<p class="label opacity-60">iFrame embed code</p>
				<pre
					class="border-surface-500/25 bg-surface-50-950/40 mt-1 overflow-x-auto rounded-lg border p-3 text-xs"><code
						>{embedCode}</code
					></pre>
				<button
					class="btn btn-sm preset-filled-secondary-500 mt-2 gap-1.5"
			...
			<div>
				...
				{#if showDirections}
					<div
						class="border-surface-500/25 bg-surface-50-950/35 mt-2 space-y-2 rounded-lg border p-3 text-sm"
						transition:slide={{ duration: 180 }}
					>
						<p class="font-medium">How to embed</p>
						<ol class="list-decimal space-y-1 pl-5">
							<li>Copy the iFrame code above.</li>
							<li>Paste it into your site page where the ride directory should appear.</li>
							<li>Adjust the `height` value if your page needs more or less space.</li>
							<li>
								Publish your page. The widget includes List, Calendar, and Map views plus an Add
								event button that links to `/ride/new`.
							</li>
						</ol>
					</div>
				{/if}
