...
<div class="mx-auto w-full max-w-5xl space-y-6 py-6">
	...
	{:else}
			...
			<div
				class={`border ${
					feedback.type === 'success'
						? 'border-secondary-500 bg-secondary-100-900/40 text-secondary-900-100'
						: feedback.type === 'info'
							? 'border-primary-500 bg-primary-50-950/30 text-primary-900-100'
							: 'border-error-500 bg-error-100-900/30 text-error-900-100'
				} rounded-lg p-4`}
			>
		...
		{#if submissionError}
			<div class="border-error-500 bg-error-100-900/30 text-error-900-100 rounded-lg border p-4">
		...
		<section class="space-y-4">
			...
			{:else}
				{#each upcomingEvents as entry (entry.event.id)}
					<article
						class="border-surface-300-700 bg-surface-100-900/60 flex flex-col gap-4 rounded-xl border p-5"
					>
						<header class="flex flex-col gap-1">
							<div class="flex flex-wrap items-center justify-between gap-3">
								<div>
									{#if entry.event.slug}
										<a
											class="text-secondary-800-200 hover:text-secondary-900-100 text-xl font-semibold"
											href={`/volunteer/${entry.event.slug}`}
										>
											{entry.event.title}
										</a>
									{:else}
										<h3 class="text-secondary-900-100 text-xl font-semibold">
											{entry.event.title}
										</h3>
									{/if}
						...
						<div class="space-y-4">
							{#each entry.assignments as record (record.id)}
								<div
									class={`border ${
										isHighlighted(record.id) ? 'border-secondary-600-400' : 'border-surface-300-700'
									} bg-surface-50-950/40 rounded-lg p-4`}
								>
									<div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
										<div class="space-y-2">
											...
											>
												...
												{:else if record.assignment.cancelled_at}
													<XCircle class="text-error-600-400 size-4" />
													<span>{record.statusLabel}</span>
												{:else}
													<span
														class="bg-primary-100-900/40 text-primary-900-100 rounded-full px-3 py-1"
													>
														{record.statusLabel}
													</span>
												{/if}
										...
										<div class="flex flex-col gap-2 md:items-end">
											...
											{/if}
											<div
												class="border-surface-300-700 bg-surface-50-950/60 mt-2 w-full rounded-lg border p-3"
											>
												<div class="flex items-center justify-between gap-2">
													...
													<button
														type="button"
														class="text-secondary-700-300 text-xs tracking-wide uppercase"
