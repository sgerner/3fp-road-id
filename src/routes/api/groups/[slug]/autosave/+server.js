...
export const POST = async ({ params, request }) => {
  ...
  if (payload.cover_file_cropped) {
    const url = await uploadDataUrlToStorage(payload.cover_file_cropped, `groups/${group_id}/cover`);
  ...
  if (Array.isArray(gt) && gt.length) {
    inserts.push(
      supabase.from('group_x_group_types').insert(gt.map((group_type_id) => ({ group_id, group_type_id })))
    );
  }
...
async function uploadDataUrlToStorage(dataUrl, destBasePath) {
  ...
  const ext = ct.includes('jpeg') ? 'jpg' : ct.includes('png') ? 'png' : ct.includes('webp') ? 'webp' : 'img';
