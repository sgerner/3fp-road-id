...
export const POST = async ({ request, fetch }) => {
	...
	if (
		...
		if (paymentIntentId) {
			const matched = await safelyFinalizePaymentIntent(paymentIntentId, fetch);
			if (!matched) {
				console.warn('Stripe webhook payment intent did not match donation records', {
					paymentIntentId,
					eventType: event.type
				});
			}
