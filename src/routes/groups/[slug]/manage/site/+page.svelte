					...
					<div class="domain-section mt-6">
						...
						<div class="mt-4">
							<DomainPurchasePanel
								...
								initialSearchQuery={data.micrositeSlug || data.group?.slug || ''}
								defaultContact={{
									email: data.group?.public_contact_email || data.group?.contact_email || '',
									city: data.group?.city || '',
									state: data.group?.state_region || '',
									country: 'US'
								}}
								returnPath={`/groups/${data.group.slug}/manage/site?domain_payment=return`}
								title="Buy Domain"
							/>
						</div>
					</div>
