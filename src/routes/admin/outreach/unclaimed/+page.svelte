...
<div class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 md:py-8">
	...
	<section class="grid gap-3 sm:grid-cols-3">
		<div class="card preset-outlined-warning p-4">
			...
			<div class="text-warning-500 text-2xl font-black">{data.stats.pending}</div>
		</div>
		<div class="card preset-outlined-primary p-4">
			<span class="text-surface-600-400 text-xs font-bold uppercase">Contacted (30d)</span>
			<div class="text-primary-500 text-2xl font-black">{data.stats.contacted}</div>
		</div>
		<div class="card preset-outlined-success p-4">
			...
			<div class="text-success-500 text-2xl font-black">{data.stats.avgCompleteness}%</div>
		</div>
	</section>
		...
		{#each groups as g}
			...
			<div class="card preset-outlined-surface space-y-4 p-4">
				...
				<div class="border-surface-500/10 flex items-center justify-between border-t pt-3">
					...
					<a
						href="/admin/outreach/group/{g.id}"
						class="btn preset-tonal-primary-500 inline-flex items-center gap-1 text-sm"
					>
						Outreach
	...
	<!-- Desktop Table -->
	<div class="card preset-outlined-surface hidden overflow-hidden md:block">
		<div class="overflow-x-auto">
			<table class="table w-full text-sm">
				<thead class="bg-surface-100-900">
					<tr>
						<th class="text-surface-600-400 px-4 py-3 text-left text-xs font-bold uppercase"
							>Group</th
						>
						<th class="text-surface-600-400 px-4 py-3 text-center text-xs font-bold uppercase"
							>Profile</th
						>
						<th class="text-surface-600-400 px-4 py-3 text-left text-xs font-bold uppercase"
							>Contacts</th
						...
						<th class="text-surface-600-400 px-4 py-3 text-left text-xs font-bold uppercase"
							>Last Contact</th
						>
						<th class="text-surface-600-400 px-4 py-3 text-right text-xs font-bold uppercase"
							>Action</th
						>
					</tr>
				</thead>
					...
					{#each groups as g}
						...
						<tr class="hover:preset-tonal-surface transition-colors">
							...
							<td class="px-4 py-3">
								{#if g.last_contact}
									<span class="text-sm"
										>{formatDistanceToNow(new Date(g.last_contact.contacted_at), {
											addSuffix: true
										})}</span
									>
									<p class="text-surface-400 text-xs">{g.last_contact.contact_method}</p>
							...
							<td class="px-4 py-3 text-right">
								<a
									href="/admin/outreach/group/{g.id}"
									class="btn preset-tonal-primary-500 group inline-flex items-center gap-1"
								>
									<span>Outreach</span>
									<IconChevronRight
