	...
	{#if shiftGroups.length}
		<div class="space-y-3">
			<header class="flex flex-col gap-1">
				<h3 class="text-surface-950-50 !text-left text-xl font-semibold">Shift Summary</h3>
				<p class="text-surface-600-400 text-sm">Review coverage by activity.</p>
			</header>
			<div class="space-y-3">
				{#each shiftGroups as group (group.id)}
					<div class="space-y-3">
						<h4
							class="text-surface-800-200 !text-left text-lg font-semibold tracking-wide uppercase"
						>
							{group.title}
						</h4>
						<div class="grid gap-3 md:grid-cols-3">
							{#each group.shifts as shift (shift.id)}
								<article
									class="bg-surface-50-950/40 border-surface-300-700/60 rounded-xl border p-4"
								>
									<div class="text-surface-700-300 text-sm">{shift.label}</div>
									<p class="text-surface-950-50 mt-2 text-xl font-semibold">{shift.approved}</p>
