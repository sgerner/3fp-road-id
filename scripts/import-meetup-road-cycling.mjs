...
function parseArgs(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (arg === '--group-limit' && next) {
			options.groupLimit = Number.parseInt(next, 10);
			index += 1;
			continue;
		}
		if (arg === '--concurrency' && next) {
			options.groupFetchConcurrency = Number.parseInt(next, 10);
			index += 1;
			continue;
		}
		if (arg === '--events-per-group' && next) {
			options.eventsPerGroup = Number.parseInt(next, 10);
			index += 1;
			continue;
		}
		if (arg === '--max-pages-per-group' && next) {
			options.maxEventPagesPerGroup = Number.parseInt(next, 10);
			index += 1;
			continue;
		}
		if (arg === '--horizon-days' && next) {
			options.horizonDays = Number.parseInt(next, 10);
			index += 1;
			continue;
		}
		if (arg === '--draft') {
		...
		if (arg === '--help') {
			console.log(`Usage: node scripts/import-meetup-road-cycling.mjs [options]

Options:
  --created-by-user-id <uuid>   Organizer user id for inserted rides
  --group-limit <n>             Limit number of Meetup groups scanned
  --concurrency <n>             Parallel Meetup group requests (default: 10)
  --events-per-group <n>        Page size for active events per group (default: 20)
  --max-pages-per-group <n>     Max pages of active events per group (default: 3)
  --horizon-days <n>            Ignore events beyond n future days (default: 365)
  --draft                       Insert rides as draft instead of published
  --dry-run                     Parse and map source without writing to the database
  --all                         Import all source events (ignore pre-filter by existing source_event_id)
...
async function main() {
	...
	const result = await importMeetupRoadCyclingTopic(supabase, options);
