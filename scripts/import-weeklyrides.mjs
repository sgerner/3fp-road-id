...
function parseArgs(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (arg === '--help') {
			console.log(`Usage: node scripts/import-weeklyrides.mjs [options]

Options:
  --created-by-user-id <uuid>   Organizer user id for inserted rides
  --draft                       Insert rides as draft instead of published
  --dry-run                     Parse and map feed without writing to the database
  --all                         Import all feed items (ignore pre-filter by existing source_event_id)
  --require-geocoding           Require geocoding (default)
  --allow-missing-geocode       Allow imports when geocoding fails
...
async function main() {
	...
	const result = await importWeeklyRidesFeed(supabase, options);
